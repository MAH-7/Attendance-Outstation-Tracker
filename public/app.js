document.addEventListener("DOMContentLoaded", function () {
  ...
  socket.on('newAttendance', (newData) => {
    // Update the appropriate tables based on the new data
    if (newData.status === "Present") {
      updatePresentTable([newData]); // Update present table with new entry
    } else if (newData.status === "Outstation") {
      updateOutstationTable([newData]); // Update outstation table with new entry
    }
  });
